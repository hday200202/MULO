...
inline Element* Container::getElement(const std::string& name) {
	for (auto& e : m_elements) {
		if (e && e->m_name == name) {
			return e;
		}
	}
	std::cerr << "UILO Warning: Element with name '" << name << "' not found in container '" << m_name << "'" << std::endl;
	Element* noElement;
	return noElement;
}

inline Element* Container::getElement(int idx) {
	if (idx < 0 || idx >= static_cast<int>(m_elements.size())) {
		std::cerr << "UILO Warning: Element index " << idx << " out of bounds in container '" << m_name << "' (size: " << m_elements.size() << ")" << std::endl;
		Element* noElement;
		return noElement;
	}
...
inline static Element* default_element = obj<Element>();
