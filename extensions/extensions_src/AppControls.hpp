...
void AppControls::init() {
    ...
    saveButton = button(
        Modifier()
            ...
            .setColor(resources->activeTheme->button_color)
            .onLClick([&](){
                uiState->saveDirectory = app->selectDirectory();
    ...
    settingsButton = button(
        Modifier()
            ...
            .setColor(resources->activeTheme->button_color)
            .onLClick([&](){
                uiState->settingsShown = !uiState->settingsShown;
                std::cout << (uiState->settingsShown ? "Show Settings" : "Hide Settings") << std::endl;
            }),
        ButtonStyle::Pill,
        "settings",
        resources->dejavuSansFont,
...
bool AppControls::handleEvents() { 
    ...
    bool currentlyPlaying = engine->isPlaying();
