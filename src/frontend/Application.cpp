...
Application::Application() {
    ...
    engine.loadComposition("assets/test_project_files/bootsncats.mpf");
    ...
    // Init MULO Components: loop until all are initialized
...
void Application::rebuildUI() {
    ...
    // Re-initialize all components with fresh references
    for (auto& [name, component] : muloComponents) {
        if (!component) {
             std::cerr << "Error: Null component created for key: " << name << std::endl;
             continue;
...
void Application::toggleFullscreen() {
    ...
    windowView.setSize({ (float)screenResolution.size.x / 2, (float)screenResolution.size.y / 2 });
    windowView.setCenter({ (float)screenResolution.size.x / 2.f, (float)screenResolution.size.y / 2.f });
    window.create(screenResolution, "MULO", sf::Style::Default, fullscreen ? sf::State::Fullscreen : sf::State::Windowed, settings);
    window.setVerticalSyncEnabled(true);
    forceUpdatePoll = 5;
}